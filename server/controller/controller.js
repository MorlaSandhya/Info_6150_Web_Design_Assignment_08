...
exports.create = (req, res) => {
    ...
    if (!req.body) {
        res = res.status(400).send({ message: "Content cannot be emtpy!" });
        res.message = "Content cannot be emtpy!";
        return;
    }
    ...
    if (!IsEmail(user.email)) {
        res.status(400).send({
            message: "Incorrect Email Address format please retry with northeastern domain"
        });
        return;
    }
